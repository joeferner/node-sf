...
sf.indent = function (str, options) {
  ...
  if (options.wordwrap) {
    var results = options.prefix;
    var prefixLength = getStringLength(options.prefix, options);
    var col = prefixLength;
    var lastSpace = -1;
    var lastSpaceSource = -1;
    var currentWord = '';
    for (var i = 0; i <= str.length; i++) {
      var char = i == str.length ? '\n' : str[i];
      ...
      if (char === '\n') {
        ...
        // if the current word doesn't fit on a line
        else if (currentWord.length > options.wordwrap - col) {
          i = lastSpaceSource;
          results = results.substr(0, lastSpace);
          results += '\n' + options.prefix;
          col = prefixLength;
          lastSpace = -1;
          lastSpaceSource = -1;
        ...
        // split the last word
          ...
          results = results.substr(0, lastSpace);
          results += '\n' + options.prefix + currentWord;
          col = prefixLength + currentWord.length;
          lastSpace = -1;
          lastSpaceSource = -1;
        }
        currentWord = '';
        ...
        if (isSpace(char)) {
          lastSpace = results.length;
          lastSpaceSource = i;
          currentWord = '';
        } else if (isSplitable(char)) {
          lastSpace = results.length + 1; // keep the character on split
          lastSpaceSource = i;
          currentWord = '';
        } else {
          currentWord += char;
